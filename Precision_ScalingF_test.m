function [OK,Xq]=Precision_ScalingF_test(Xi,yi, Sel_Feat)
% check inputs
clc
...
for j=1:L 
    Xq=zeros(size(X));
    for p=1:nr
              [Xq(:,p)] = Precision_ScalingF(X(:,p), numLevels(j),1);
    end
...
cv = cvpartition(length(y), 'KFold', k);
 for i = 1:k
        ...
        X_test = X(test(cv, i), :);
        y_test = y(test(cv, i));
       [accuracyS(i), precisionS(i), recallS(i), f1_scoreS(i),cm1,tElapsed1(i), Noitr(i),NoSVs(i)]=SVM_RUN(X_train,y_train,X_test,y_test, itrN);
        cmS(:,:,i)=cm1;
