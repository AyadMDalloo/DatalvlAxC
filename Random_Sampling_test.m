function [OK,X,y]=Random_Sampling_test(Xi,yi,Pre_Reduce, Sel_Feat)
% check inputs
clc
...
[Xi]=Normalize_Standardize(Xi,'N');
% Xi=X_NoS;
%% 
%Pre_Reduce=[1 0.95 0.9 0.85 0.8 0.75];
%Pre_Reduce=[1:-.05:0.5];
% Pre_Reduce=[0.7];
L=length(Pre_Reduce);
...
for j=1:L 
    ...
    [X, y]=RandomSampling(Xi,yi,Sel_Feat,Pre_Reduce(j));
%   assignin('base', 'X', X); 
%   assignin('base', 'y', y); 
cv = cvpartition(length(y), 'KFold', k);
    ...
    for i = 1:k
        ...
        [accuracyS(i), precisionS(i), recallS(i), f1_scoreS(i),cm1,tElapsed1(i), Noitr(i),NoSVs(i)]=SVM_RUN(X_train,y_train,X_test,y_test,100);
        cmS(:,:,i)=cm1;
