function [OK]=systematic_Sampling_test(Xi,yi,Pre_Reduce, Sel_Feat)
% check inputs
clc
 if nargin < 2  % Check if both arguments are provided
        error('arguments are required for this function.');
 end
if ~exist('Sel_Feat', 'var') || isempty(Sel_Feat) % % Check if the argument e_xists or is empty 
       Sel_Feat=size(Xi,2);
       Sel_Feat=1:Sel_Feat;
end
...
%assignin('base', 'X_NoS', X_NoS); 
%% 
% Pre_Reduce=0.4;
%Pre_Reduce=[1 0.4  0.3];
Xi=Xi(:,Sel_Feat);
L=length(Pre_Reduce);
...
for j=1:L 
    disp(Pre_Reduce(j))
    [X, y]= systematicSampling(Xi,yi, Pre_Reduce(j));
  assignin('base', 'Xo', X); 
%   assignin('base', 'y', y); 
